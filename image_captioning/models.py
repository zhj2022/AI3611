...
class DecoderWithAttention(nn.Module):
    ...
    def forward(self, encoder_out, encoded_captions, caption_lengths, schedule_eps):
        """
        Forward propagation.
        ...
        for t in range(max(decode_lengths)):
            ...
            if t == 0 or torch.rand(1) < schedule_eps: # groud_truth
                embeds = embeddings[:batch_size_t, t, :]
            else: # our generation
                embeds = self.embedding(prev)[:batch_size_t, 0]
            # [batch_size_t, decoder_dim]
            h, c = self.decode_step(
                torch.cat([embeds, attention_weighted_encoding], dim=1),
                (h[:batch_size_t], c[:batch_size_t]))
            # [batch_size_t, vocab_size]
            preds = self.fc(self.dropout(h))
            predictions[:batch_size_t, t, :] = preds
            alphas[:batch_size_t, t, :] = alpha
            prev = preds.argmax(1).unsqueeze(1)
...
class Captioner(nn.Module):
    ...
    def forward(self, images, encoded_captions, caption_lengths, schedule_eps):
        """
        ...
        decoder_out = self.decoder(encoder_out, encoded_captions,
                                   caption_lengths.unsqueeze(1), schedule_eps)
        return decoder_out
