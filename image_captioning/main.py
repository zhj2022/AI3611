...
class Runner(object): 
    ...
    def train_model(self, train_loader, model, loss_fn, optimizer, desc='', schedule_eps=1):
        running_acc = 0.0
        running_loss = 0.0
        model.train()
        ...
        for batch_idx, batch in enumerate(t):
            ...
            scores, caps_sorted, decode_lengths, alphas, sort_ind = model(
                images, captions, lengths, schedule_eps)
    ...
    def train(self, config_file, **kwargs):
        ...
        model_path = os.path.join(args["outputpath"],
            ...
            f"emd{args['embedding_dim']}_sche{args['schedule_sampling']}_k{args['sample_k']}")
        ...
        for epoch in range(num_epochs):
            if schedule_sampling == "no":
                eps = 1
            elif schedule_sampling == "linear":
                eps = max(0.1, 1 - epoch * k)
            elif schedule_sampling == "exp":
                eps = k ** epoch
            elif schedule_sampling == "sigmoid":
                eps = k / (k + np.exp(epoch / k))
            else:
                print("schedule sampling method not supported, reset as default\n")
                eps = 1
